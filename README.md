SmartAsk（智问）是一个企业级的 AI 知识库管理系统，采用检索增强生成（RAG）技术，提供智能文档处理和检索能力。
...
├── SmartAskApplication.java      # 主应用程序入口
├── client/                       # 外部API客户端
├── config/                       # 配置类
├── consumer/                     # Kafka消费者
...
```bash
frontend/
├── packages/           # 可重用模块（Monorepo workspace）
│   ├── axios/          # HTTP客户端封装
│   ├── hooks/          # 通用Vue hooks
│   ├── materials/      # UI组件库
│   ├── color/          # 颜色处理工具
...
智问提供了完整的文档上传与解析功能，支持文件分片上传和断点续传，并支持标签进行组织管理。文档可以是公开的，也可以是私有的，并且可以与特定的组织标签关联，以便更好地进行权限分类。
...
智问通过组织标签支持多租户架构。每个用户可以创建或加入一个或多个组织，每个组织可以拥有独立的知识库和文档管理。这样，企业可以在同一系统中管理多个团队或部门的知识库，而无需担心数据混淆或权限问题。
...
- **前端实现**：使用 `@vueuse/core` 的 `useWebSocket`，自动重连（3 秒间隔，最多 5 次）
- **后端处理**：`ChatWebSocketHandler` 处理连接，JWT 从 URL 路径提取用户身份
...
- **开发环境**：`frontend/.env.test`，API 基础地址 `http://localhost:8081/api/v1`
- **生产环境**：`frontend/.env.prod`

### 后端环境

- **主配置**：`src/main/resources/application.yml`
- **开发环境**：`src/main/resources/application-dev.yml`
- **Docker 环境**：`src/main/resources/application-docker.yml`
...
智问的架构具备一个现代化的、云原生应用程序的特点，具有清晰的关注点分离、可扩展的组件和与 AI 技术的集成。模块化设计允许随着技术的发展，特别是快速变化的 AI 集成领域，未来可以扩展和替换单个组件。

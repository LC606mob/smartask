...
const username = computed(() => props.msg.username || authStore.userInfo.username);
  ...
  <div class="mb-8 flex-col gap-2">
    <div v-if="msg.role === 'user'" class="flex items-center gap-4">
      ...
      <div class="flex-col gap-1">
        <NText class="text-4 font-bold">{{ username }}</NText>
    ...
    <div v-else class="flex items-center gap-4">
      ...
      <div class="flex-col gap-1">
        <NText class="text-4 font-bold">智问</NText>
        <NText class="text-3 color-gray-500">{{ formatDate(msg.timestamp) }}</NText>
      </div>
    </div>

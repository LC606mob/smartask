 ...
 * 【职责】定义应用的安全规则和过滤链
 * 【设计思路】
 *   - 无状态会话（STATELESS），适合RESTful API
 *   - JWT认证过滤器：从请求头提取Token，验证用户身份
 *   - 组织标签过滤器：提取用户的组织标签信息，用于权限控制
 *
 * 【权限规则】
 *   - 公开接口：登录、注册、WebSocket、静态资源
 *   - 用户接口：文件上传、搜索、对话历史
 *   - 管理员接口：用户管理、组织标签管理
 *
 * 【过滤链顺序】
 *   请求 → JwtAuthenticationFilter → OrgTagAuthorizationFilter → Controller
 */
@Configuration
@EnableWebSecurity

package com.lcmob.smartask.consumer;
...
import com.lcmob.smartask.service.VectorizationService;
 ...
 * 文件处理消费者
 *
 * 【职责】消费Kafka消息，触发文件解析和向量化
 * 【设计思路】
 *   - 监听file-processing-topic1主题
 *   - 下载文件 → 解析文档 → 向量化 → 存储到ES
 *   - 支持失败重试（最多4次）和死信队列
 *
 * 【处理流程】
 *   1. 接收FileProcessingTask消息
 *   2. 从MinIO下载文件
 *   3. 调用ParseService解析文档（提取文本，分割分块）
 *   4. 调用VectorizationService向量化（生成向量，存储到ES）
 *
 * 【错误处理】
 *   - 失败时抛出异常，由Kafka的DefaultErrorHandler处理
 *   - 重试4次（间隔3秒），失败后发送到死信队列
 *
 * 【调用链】
 *   Kafka → FileProcessingConsumer → ParseService（解析）

 ...
 * 【设计思路】
 *   采用双Token机制：
 *   - accessToken：短期有效（2小时），用于API认证
 *   - refreshToken：长期有效（7天），用于刷新accessToken
 *
 * 【为什么需要双Token】
 *   - accessToken短期有效，即使泄露，影响时间有限
 *   - refreshToken长期有效，但只用于刷新token，不用于API认证
 *   - 用户无感知地刷新token，保持登录状态
 *
 * 【调用链】
 *   前端检测token即将过期 → /auth/refreshToken → 生成新token
 */
@RestController

...
import com.lcmob.smartask.service.DocumentService;
 ...
 * 【设计思路】
 *   - 删除：只有文件所有者或管理员可以删除，删除时同步清理MinIO、ES、MySQL
 *   - 查询：支持两种视图 - "可访问文件"（含公开+组织文档）和"我的上传"
 *   - 下载/预览：通过MinIO预签名URL实现，有效期1小时
 *
 * 【权限模型】
 *   用户可访问的文件 = 自己上传的 + 公开的 + 同组织的
 *
 * 【调用链】
 *   前端 → DocumentController → DocumentService → MinIO（文件存储）
 *                                                → Elasticsearch（索引删除）
 *                                                → MySQL（记录删除）
 */
@RestController

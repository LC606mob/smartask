...
import com.lcmob.smartask.service.AuthService;
 ...
 * 用户控制器
 *
 * 【职责】处理用户注册、登录、登出、个人信息管理等操作
 * 【设计思路】
 *   - 注册：创建用户账号，密码加密存储
 *   - 登录：验证身份，生成双Token（accessToken + refreshToken）
 *   - 登出：支持单设备登出和全设备登出（token失效机制）
 *   - 个人信息：查询/修改用户资料、组织标签
 *
 * 【安全机制】
 *   - 密码使用BCrypt加密存储
 ...
 *
 * 【调用链】
 *   前端 → UserController → AuthService（认证）
...
public class UserController {
    ...
    public ResponseEntity<?> login(@RequestBody UserRequest request) {
        ...
        try {
            ...
            String username = authService.authenticateUser(request.username(), request.password());
            if (username == null) {
    ...
    public ResponseEntity<?> getUserOrgTags(@RequestHeader("Authorization") String token) {
        ...
        try {
            ...
            Map<String, Object> orgTagsInfo = userProfileService.getUserOrgTags(username);
    ...
    public ResponseEntity<?> setPrimaryOrg(@RequestHeader("Authorization") String token, @RequestBody PrimaryOrgRequest request) {
        ...
        try {
            ...
            userProfileService.setUserPrimaryOrg(username, request.primaryOrg());
    ...
    public ResponseEntity<?> getUploadOrgTags(@RequestAttribute("userId") String userId) {
        ...
        try {
            ...
            // 获取用户所有组织标签
            List<String> orgTags = Arrays.asList(userProfileService.getUserOrgTags(userId).get("orgTags").toString().split(","));
            // 获取用户主组织标签

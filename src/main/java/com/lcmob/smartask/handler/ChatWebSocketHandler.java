package com.lcmob.smartask.handler;
 ...
 * WebSocket聊天处理器
 *
 * 【职责】处理WebSocket连接、消息和关闭事件
 * 【设计思路】
 *   - 连接时从URL路径提取JWT Token，验证用户身份
 *   - 消息时转发给ChatHandler处理
 *   - 支持停止指令（前端可以中断AI响应）
 *
 * 【连接地址】
 *   ws://host/chat/{jwtToken}
 *
 * 【消息类型】
 *   - 普通文本：用户的问题
 *   - JSON指令：停止响应（包含内部令牌验证）
 *
 * 【调用链】
 *   前端WebSocket → ChatWebSocketHandler → ChatHandler（处理消息）
 */
@Component

 ...
 * 文件类型验证服务
 *
 * 【职责】验证上传的文件类型是否支持解析和向量化
 * 【设计思路】
 *   - 在文件上传的第一个分片时进行验证
 *   - 尽早失败，避免后续分片白白上传
 *
 * 【支持的文件类型】
 *   - 文档：PDF、Word、Excel、PowerPoint、文本、Markdown
 *   - 网页：HTML、XML、JSON、CSV
 *   - 电子书：EPUB
 *   - 其他：OpenDocument、Apple iWork
 *
 * 【不支持的文件类型】
 *   - 图片：JPG、PNG、GIF等
 *   - 音视频：MP3、MP4、AVI等
 ...
 * 【为什么只支持文档类型】
 *   - 只有文档类型包含可提取的文本内容
 *   - 图片/音视频需要OCR/语音识别，复杂度高
 *   - 当前版本专注于文档知识库场景
 *
 * 【调用链】
 *   UploadController → FileTypeValidationService（验证文件类型）
 */
@Service

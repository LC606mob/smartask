 ...
 * 组织标签缓存服务
 *
 * 【职责】缓存用户的组织标签信息，提高权限验证效率
 * 【设计思路】
 *   - 使用Redis缓存用户的组织标签和主组织
 *   - 支持有效标签计算（包含父标签继承）
 *   - 缓存有效期24小时
 *
 * 【为什么需要缓存】
 ...
 *
 * 【缓存结构】
 *   - user:org_tags:{username}：用户的直接组织标签
 *   - user:primary_org:{username}：用户的主组织标签
 *   - user:effective_org_tags:{username}：用户的有效标签（含父标签）
 *
 * 【有效标签计算】
 *   用户的有效标签 = 直接标签 + 所有父标签 + 默认标签
 *   例如：用户属于"技术部"，"技术部"的父标签是"公司"
 *   则有效标签 = ["技术部", "公司", "DEFAULT"]
 *
 * 【调用链】
 *   HybridSearchService → OrgTagCacheService（权限验证）
 *   DocumentService → OrgTagCacheService（文件查询）
 */
@Service

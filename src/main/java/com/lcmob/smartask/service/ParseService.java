 ...
 * 文档解析服务
 *
 * 【职责】将上传的文档解析为文本，并分割为语义完整的分块
 * 【设计思路】
 *   采用"父文档-子切片"策略：
 *   1. 使用Apache Tika解析各种格式的文档（PDF、Word、Excel等）
 *   2. 流式处理，避免大文件导致OOM
 *   3. 将文本按1MB分割为"父块"
 *   4. 每个父块再按512字符分割为"子切片"
 *   5. 使用HanLP进行中文分词，保持语义完整性
 *
 * 【为什么用父子文档策略】
 *   - 父块（1MB）：保留较大的上下文，用于BM25文本搜索
 *   - 子切片（512字符）：精确的语义单元，用于向量搜索
 *   - 两者结合，兼顾上下文和精确性
 *
 * 【分块策略】
 *   1. 按段落分割（\n\n）
 ...
 *
 * 【调用链】
 *   Kafka消费者 → ParseService → Apache Tika（文档解析）
 *                              → HanLP（中文分词）
 *                              → MySQL（存储分块）
 */
@Service

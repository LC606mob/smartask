 ...
 * Token缓存服务
 *
 * 【职责】基于Redis实现JWT Token的状态管理
 * 【设计思路】
 *   - 支持Token主动失效（登出时）
 *   - 支持黑名单机制（防止已登出的Token继续使用）
 *   - 支持批量登出（失效用户的所有Token）
 *
 * 【为什么需要Token状态管理】
 *   - JWT本身是无状态的，一旦签发无法主动失效
 *   - 通过Redis记录Token状态，实现主动失效
 *   - 支持安全的登出功能
 *
 * 【缓存结构】
 *   - jwt:valid:{tokenId}：有效的Token信息
 *   - jwt:refresh:{refreshTokenId}：Refresh Token信息
 *   - jwt:blacklist:{tokenId}：Token黑名单
 *   - jwt:user:{userId}:tokens：用户的活跃Token集合
 *
 * 【Token生命周期】
 *   1. 登录时：生成Token，缓存到Redis
 *   2. 请求时：验证Token是否在黑名单中
 *   3. 登出时：将Token加入黑名单
 *   4. 过期时：自动从Redis删除
 *
 * 【调用链】
 *   JwtUtils → TokenCacheService → Redis
 */
@Service
